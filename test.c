// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.

#ifndef _WIN32
#include <wsl/winadapter.h>
#endif

#include <directx/d3d12.h>

#ifndef _WIN32
#include <unistd.h>
#include <dlfcn.h>
#endif

#include "test.h"

const unsigned char g_cs[] = {
  
};

#define VERIFY_SUCCEEDED(expr) hr = (expr); if (FAILED(hr)) { printf("FAILED: %s: 0x%x\n", #expr, hr); return hr; } else { printf("Succeeded: %s\n", #expr); }
#define VERIFY_ARE_EQUAL(a, b) if (a != b) { printf("NOT EQUAL: %d, %d\n", a, b); return 1; } else { printf("Equal: %s and %s\n", #a, #b); }
int main()
{
    typedef HRESULT (*FnD3D12CreateDevice)(IUnknown *, D3D_FEATURE_LEVEL, REFIID, void **);
#ifdef _WIN32
    auto dxcore = LoadLibraryA("dxcore.dll");
    auto d3d12 = LoadLibraryA("d3d12.dll");
    auto pfnDXCoreCreateAdapterFactory = (FnDXCoreCreateAdapterFactory)GetProcAddress(dxcore, "DXCoreCreateAdapterFactory");
    auto pfnD3D12CreateDevice = (decltype(&D3D12CreateDevice))GetProcAddress(d3d12, "D3D12CreateDevice");

#else
    void *dxcore = dlopen("libdxcore.so", RTLD_NOW | RTLD_LOCAL);
    void *d3d12 = dlopen("libd3d12.so", RTLD_NOW | RTLD_LOCAL);
    FnDXCoreCreateAdapterFactory pfnDXCoreCreateAdapterFactory = (FnDXCoreCreateAdapterFactory)dlsym(dxcore, "DXCoreCreateAdapterFactory");
    FnD3D12CreateDevice pfnD3D12CreateDevice = (FnD3D12CreateDevice)dlsym(d3d12, "D3D12CreateDevice");
#endif
    
    HRESULT hr = S_OK;

    ID3D12Device *pDevice = NULL;
    VERIFY_SUCCEEDED(pfnD3D12CreateDevice(GetDXCoreAdapter(pfnDXCoreCreateAdapterFactory), D3D_FEATURE_LEVEL_1_0_CORE, &IID_ID3D12Device, (void **)&pDevice));

    ID3D12Resource *pUAV = NULL, *pStaging = NULL;
    D3D12_RESOURCE_DESC UAVDesc = {0};
    UAVDesc.Width = 65536;
    UAVDesc.Height = 1;
    UAVDesc.DepthOrArraySize = 1;
    UAVDesc.MipLevels = 1;
    UAVDesc.Format = DXGI_FORMAT_UNKNOWN;
    UAVDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
    UAVDesc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
    UAVDesc.SampleDesc.Count = 1;
    UAVDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;

    D3D12_HEAP_PROPERTIES UAVHeapDesc = {.Type = D3D12_HEAP_TYPE_DEFAULT};

    D3D12_RESOURCE_DESC StagingDesc = UAVDesc;
    StagingDesc.Flags = D3D12_RESOURCE_FLAG_NONE;

    D3D12_HEAP_PROPERTIES StagingHeapDesc = {.Type = D3D12_HEAP_TYPE_READBACK};

    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateCommittedResource(pDevice, &UAVHeapDesc, D3D12_HEAP_FLAG_SHARED, &UAVDesc, D3D12_RESOURCE_STATE_COMMON, NULL, &IID_ID3D12Resource, (void **)&pUAV));
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateCommittedResource(pDevice, &StagingHeapDesc, D3D12_HEAP_FLAG_NONE, &StagingDesc, D3D12_RESOURCE_STATE_COMMON, NULL, &IID_ID3D12Resource, (void **)&pStaging));

    D3D12_RESOURCE_DESC UAVDescCopy = pUAV->lpVtbl->GetDesc(pUAV);
    VERIFY_ARE_EQUAL((UINT)UAVDescCopy.Width, (UINT)UAVDesc.Width);

    ID3D12RootSignature *pRootSig;
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateRootSignature(pDevice, 0, g_cs, sizeof(g_cs), &IID_ID3D12RootSignature, (void **)&pRootSig));

    ID3D12PipelineState *pPSO;
    D3D12_COMPUTE_PIPELINE_STATE_DESC PSODesc = {0};
    PSODesc.CS.pShaderBytecode = g_cs;
    PSODesc.CS.BytecodeLength = sizeof(g_cs);
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateComputePipelineState(pDevice, &PSODesc, &IID_ID3D12PipelineState, (void **)&pPSO));

    ID3D12CommandAllocator *pAllocator;
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateCommandAllocator(pDevice, D3D12_COMMAND_LIST_TYPE_COMPUTE, &IID_ID3D12CommandAllocator, (void **)&pAllocator));

    ID3D12GraphicsCommandList *pCommandList;
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateCommandList(pDevice, 0, D3D12_COMMAND_LIST_TYPE_COMPUTE, pAllocator, pPSO, &IID_ID3D12GraphicsCommandList, (void **)&pCommandList));
    pCommandList->lpVtbl->SetComputeRootSignature(pCommandList, pRootSig);
    pCommandList->lpVtbl->SetComputeRootUnorderedAccessView(pCommandList, 0, pUAV->lpVtbl->GetGPUVirtualAddress(pUAV));
    pCommandList->lpVtbl->SetPipelineState(pCommandList, pPSO);
    pCommandList->lpVtbl->Dispatch(pCommandList, 1, 1, 1);

    D3D12_RESOURCE_BARRIER barrier = {
        .Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION,
        .Transition = {
            .pResource = pUAV,
            .StateBefore = D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
            .StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE,
            .Subresource = 0
        }
    };
    pCommandList->lpVtbl->ResourceBarrier(pCommandList, 1, &barrier);

    pCommandList->lpVtbl->CopyResource(pCommandList, pStaging, pUAV);
    VERIFY_SUCCEEDED(pCommandList->lpVtbl->Close(pCommandList));

    ID3D12CommandQueue *pQueue;
    D3D12_COMMAND_QUEUE_DESC CQDesc = { D3D12_COMMAND_LIST_TYPE_COMPUTE };
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateCommandQueue(pDevice, &CQDesc, &IID_ID3D12CommandQueue, (void **)&pQueue));
    pQueue->lpVtbl->ExecuteCommandLists(pQueue, 1, (ID3D12CommandList *const *)&pCommandList);

    ID3D12Fence *pFence;
    VERIFY_SUCCEEDED(pDevice->lpVtbl->CreateFence(pDevice, 0, D3D12_FENCE_FLAG_NONE, &IID_ID3D12Fence, (void **)&pFence));
    VERIFY_SUCCEEDED(pQueue->lpVtbl->Signal(pQueue, pFence, 1));

    VERIFY_SUCCEEDED(pFence->lpVtbl->SetEventOnCompletion(pFence, 1, NULL));
    
    //VERIFY_ARE_EQUAL(pFence->lpVtbl->GetCompletedValue(pFence), 1ull);

    void* pData = NULL;
    VERIFY_SUCCEEDED(pStaging->lpVtbl->Map(pStaging, 0, NULL, &pData));

    VERIFY_ARE_EQUAL(*(UINT*)pData, 0xd3d12u);
}


